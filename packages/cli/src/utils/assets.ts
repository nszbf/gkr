...
import { ActionOnFile, CLIAppConfig } from '../types';

export class AssetsManager {
    constructor(
        protected config: CLIAppConfig,
    ...
    copyAssets() {
        if (this.config.compile.assets.length <= 0) {
            return this;
        }

        try {
            const filesToCopy = this.config.compile.assets.map((item) => {
                if (typeof item === 'string') {
                    return {
                        glob: join(this.config.paths.app, item),
                        outDir: this.config.paths.output,
                    };
                }
                return {
                    outDir: item.outDir ?? this.config.paths.output,
                    glob: join(this.config.paths.app, item.include!),
                    exclude: item.exclude ? join(this.config.paths.app, item.exclude) : undefined,
            ...
            const isWatchEnabled = this.onWatch;
    ...
    private actionOnFile(option: ActionOnFile) {
        ...
        const target = relative(this.config.paths.app, path);

        const dest = join(
            this.config.paths.app,
            item.outDir!,
