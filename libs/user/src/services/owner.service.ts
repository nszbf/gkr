...
export class OwnerService {
    ...
    async validate(context: ExecutionContext) {
        ...
        if (Array.isArray(meta.resource)) {
            return meta.resource.every(
                async (item) => await this.check({ resource: item, owner: meta.owner }, request),
            );
        }
        return this.check({ resource: meta.resource, owner: meta.owner }, request);
    }
    ...
    protected async check(
        ...
        resource.ownerResourceName = resource.ownerResourceName ?? lowerFirst(owner.model.name);
        // 默认所属者资源查询字段为id
