...
export class UniqueTreeExistConstraint implements ValidatorConstraintInterface {
    async validate(value: any, args: ValidationArguments) {
        const { entity, ignore = 'id', findKey, property } = args.constraints[0] as IParams;
        // 需要查询的属性名,默认为当前验证的属性
        ...
        return !rows.find((row) => row[queryProperty] === value && row[ignore] !== ignoreValue);
    }
...
export function IsTreeUniqueExist(params: IParams, validationOptions?: ValidationOptions) {
